      ...
      <ul id="events-list" className="mt-2 text-sm max-h-72 overflow-y-auto border border-zinc-800 rounded p-2 bg-zinc-900/50 scroll-smooth">
        {events.map((e) => (
          <li key={e.id} className="border-b last:border-0 py-1.5 border-zinc-800/50">
            <code className="text-zinc-500">{formatTimeIST(e.created_at)}</code> · <b className={e.kind.includes("blocked") || e.kind.includes("exit") || e.kind === "terminated" || e.kind.includes("detected") ? "text-red-500" : "text-blue-400"}>{e.kind}</b>
